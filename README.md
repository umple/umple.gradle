...
*Tasks:*
  - `compile{source set name}Umple` (or simply `compileUmple` if you're using the `main` source set)
...
The plugin has four configuration properties that are used to customize the `compile{source set name}Umple` task.
  ...
  - `customMasterPath`: If you want the plugin to look for a master file in a location that doesn't conform to the `{project root directory}/src/{source set name}/umple` convention, set this flag to true and provide as the `master` value the path to the master file you want to use. This path must start from the root directory of the project that contains the `master` file you want to use.
